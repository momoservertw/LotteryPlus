...
public class TabComplete implements TabCompleter {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        switch (args.length) {
            case 1:
                if (CorePlusAPI.getPlayerManager().hasPerm(ConfigHandler.getPluginName(), sender, "lotteryplus.use")) {
                    commands.add("help");
                }
                if (CorePlusAPI.getPlayerManager().hasPerm(ConfigHandler.getPluginName(), sender, "lotteryplus.command.reload")) {
                    commands.add("reload");
                }
                if (CorePlusAPI.getPlayerManager().hasPerm(ConfigHandler.getPluginName(), sender, "lotteryplus.command.version")) {
                ...
                break;
            case 2:
                if (args[0].equalsIgnoreCase("lottery") && CorePlusAPI.getPlayerManager().hasPerm(ConfigHandler.getPluginName(), sender, "lotteryplus.command.lottery")) {
