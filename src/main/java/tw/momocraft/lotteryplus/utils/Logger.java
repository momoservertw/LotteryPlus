...
public class Logger {
    ...
    public void createDefaultLog() {
        File folder = new File(LotteryPlus.getInstance().getDataFolder().getPath() + "\\Logs");
        // Check log folder.
        if (!folder.exists()) {
            try {
                if (!folder.mkdir()) {
    ...
    public void createCustomLog() {
        ...
        if (path.startsWith("plugin//")) {
            path = path.replace("plugin/", LotteryPlus.getInstance().getDataFolder().getPath());
    ...
    public void createLotteryLog() {
        File folder = new File(LotteryPlus.getInstance().getDataFolder().getPath() + "\\LotteryLogs");
        // Check log folder.
        if (!folder.exists()) {
            try {
                if (!folder.mkdir()) {
        ...
        if (!lotteryFile.exists()) {
            ...
            if (ConfigHandler.getConfigPath().isLotteryLogNew()) {
                Date modifiedDate = new Date(lotteryFile.lastModified());
                Date currentDate = new Date();
                // Exist old log.
                if (modifiedDate.equals(currentDate)) {
                    // Rename old log file to "2020-11-10".
                    String logPath = lotteryFile.getParentFile().getPath() + "\\" + new SimpleDateFormat("yyyy-MM-dd").format(modifiedDate);
                    File renameFile = new File(logPath + ".log");
                    String logName;
                    int number = 1;
                    while (renameFile.exists()) {
                    ...
                    try {
                        if (!lotteryFile.renameTo(renameFile)) {
                            ServerHandler.sendConsoleMessage("&6Log: &frename old log &8\"&e" + renameFile.getName() + "&8\"  &c✘");
                        }
                    } catch (Exception e) {
                        ServerHandler.sendDebugTrace(e);
     ...
     */
    public void addLotteryLog(String message, boolean time) {
        message = message + "\n";
        if (time) {
            DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
            String date = dateFormat.format(new Date());
        ...
        try {
            BufferedWriter bw = new BufferedWriter(new FileWriter(lotteryFile, true));
            bw.append(message);
            bw.close();
        } catch (IOException e) {
            ServerHandler.sendDebugTrace(e);
